    ...
    def __getitem__(self, index):
        ...
        for j in range(ACTION_HORIZON):
            if frame + j < table.num_rows:
                ...
                valid[j] = 1.0
        videos, fps, spatial_images_current = {}, {}, {}
        for view in VIEWS:
            ...
            raw_frames = reader.get_batch(history).asnumpy()
            videos[view] = [center_crop(x) for x in raw_frames]
            # Spatial Forcing v2 uses the unmodified current RGB frame.  Keep
            # the policy video history above unchanged and pass this tensor
            # through the adapter so the collate step can batch it safely.
            spatial_images_current[view] = raw_frames[-1]
            fps[view] = float(reader.get_avg_fps())
        return {"sample_id": row["sample_id"], "task": row["task"], "instruction": row["instruction"], "frame": frame, "videos": videos, "fps": fps, "spatial_images_current": spatial_images_current, "state": states, "action": action, "valid": valid}
...
def spatial_messages(item):
    """One-frame current-only Qwen inputs for the v2 alignment branch."""
    content = []
    for index, (label, view) in enumerate((("Left camera: ", LEFT), ("Right camera: ", RIGHT), ("Wrist camera: ", WRIST))):
        if index:
            content.append({"type": "text", "text": "\n"})
        # The alignment student follows the official XR-1 spatial pipeline:
        # current frame, after the same 0.95 center crop used by policy Qwen.
        frame = item["videos"][view][-1]
        content.extend(({"type": "text", "text": label}, {"type": "video", "video": [frame], "fps": item["fps"][view]}))
    content.append({"type": "text", "text": "\n\nSpatial observation only: " + str(item["instruction"])})
...
class OfficialSampleDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        current = torch.stack([
            torch.from_numpy(np.asarray(raw["spatial_images_current"][view], dtype=np.uint8)).permute(2, 0, 1).contiguous()
            for view in VIEWS
        ])
        if current.ndim != 4 or current.shape[1] != 3:
            raise RuntimeError(f"current spatial RGB must be [3,3,H,W], got {tuple(current.shape)}")
        extra = {"spatial_messages": spatial_messages(raw), "spatial_images_current": current}
        return {"messages": official_messages(raw), "action": target, "action_mask": valid_mask, "state": torch.from_numpy(raw["state"]), "vlm_action_target": target[:steps], "vlm_action_mask": valid_mask[:steps], "vlm_action_actual_length": steps, "sample_id": raw["sample_id"], "task": raw["task"], **extra}


from mibot.data.collate.custom_collate import CustomCollate as _CleanCustomCollate, IM_START_ID, STATE_ID
...
class LocalOfficialCollate(_CleanCustomCollate):
    ...
    def _prepare(self, item, key):
        inputs = self.processor.apply_chat_template(
            item[key], tokenize=True, return_dict=True, return_tensors="pt", do_resize=False,
        )
        inputs["position_ids"] = self._position_ids(inputs)
        return inputs

    @staticmethod
    def _pack_current(inputs_list):
        lengths = [int(x["input_ids"].shape[1]) for x in inputs_list]
        offsets = [0] + list(itertools.accumulate(lengths))
        out = {
            "input_ids": torch.cat([x["input_ids"] for x in inputs_list], dim=1),
            "position_ids": torch.cat([x["position_ids"] for x in inputs_list], dim=2),
            ...
            "segments": torch.tensor(list(zip(offsets[:-1], offsets[1:])), dtype=torch.long),
        }
        for key in ("pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw"):
            values = [x[key] for x in inputs_list if x.get(key) is not None]
            if values:
    ...
    def __call__(self, batch):
        ...
        total_length = 0
        for item in batch:
            inputs = self._prepare(item, "messages")
            ...
            spatial = self._prepare(item, "spatial_messages")
            selected.append((inputs, spatial, {key: value for key, value in item.items() if key not in ("messages", "spatial_messages")}, int(im_starts[-1])))
            total_length += length
        if not selected:
            raise ValueError(f"no training sample fits within MAX_LENGTH={self.max_length}")

        inputs_list = [x[0] for x in selected]
        lengths = [int(x["input_ids"].shape[1]) for x in inputs_list]
        offsets = [0] + list(itertools.accumulate(lengths))
        result = {
            "input_ids": torch.cat([x["input_ids"] for x in inputs_list], dim=1),
            ...
            "action_vlm_condition_segments": torch.tensor([[offsets[i], offsets[i] + end] for i, (_, _, _, end) in enumerate(selected)], dtype=torch.long),
        }
        for key in ("pixel_values", "image_grid_thw", "pixel_values_videos", "video_grid_thw"):
            values = [x[0][key] for x in selected if x[0].get(key) is not None]
            if values:
                result[key] = torch.cat(values)
        payloads = [x[2] for x in selected]
        ...
        result["spatial_batch"] = self._pack_current([x[1] for x in selected])
        return result
...
def write_audit(out: Path, processor, rows, args):
    ...
    (out / "training_contract.json").write_text(json.dumps({"max_steps": int(args.max_steps), "val_check_interval_optimization_steps": 1000, "save_every_optimization_steps": 1000, "gradient_accumulation": 1, "per_gpu_batch_size": int(args.batch_size), "gpu_prefetch": bool(args.gpu_prefetch), "gpu_allocator_limit_gib": float(args.limit_gpu_gb)}, indent=2) + "\n")
