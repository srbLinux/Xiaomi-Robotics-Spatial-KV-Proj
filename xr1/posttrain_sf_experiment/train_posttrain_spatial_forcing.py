...
path from ``train_posttrain_baseline``.  The only trainable policy
...
import train_posttrain_baseline as common
...
def load_six_rows(manifest: Path):
    common.MANIFEST = Path(manifest)
    common.META = common.MANIFEST.parent / "train.meta.json"
    ...
    for task in TASKS:
        val = {(r["task"], int(r["episode_id"])) for r in rows["val"] if r["task"] == task}
        if len(val) != 2:
...
class ForcingRunner(common.Official20Runner):
    ...
    def configure_optimizers(self):
        # Reuse the exact official BaseRunner optimizer/scheduler.  The
        # trainable scope remains the SF LoRA/projector scope configured above.
        result = super().configure_optimizers()
        optimizer = result["optimizer"]
        optimizer_ids = {id(parameter) for group in optimizer.param_groups for parameter in group["params"]}
        expected_ids = {id(parameter) for parameter in self.model.parameters() if parameter.requires_grad}
        if optimizer_ids != expected_ids:
            raise RuntimeError(
                f"SF optimizer scope mismatch: missing={len(expected_ids - optimizer_ids)} "
                f"extra={len(optimizer_ids - expected_ids)}"
            )
        return result
    ...
    def _forward(self, batch):
        ...
        # The complete official policy objective is shared with baseline.
        ...
        if not self.sf:
            ...
            return result
        projected = F.normalize(self.model.spatial_alignment_projector(student).float(), dim=-1)
        normalized_teacher = F.normalize(teacher.detach().float(), dim=-1)
        cosine = (projected * normalized_teacher).sum(-1)
        align = (1.0 - cosine).mean()
        result["loss_official"] = policy_loss
        result["loss_align"] = align
        result["loss_align_left"] = (1.0 - cosine[:, 0]).mean()
        result["loss_align_right"] = (1.0 - cosine[:, 1]).mean()
        result["loss_align_wrist"] = (1.0 - cosine[:, 2]).mean()
...
def write_contract(out: Path, args, rows, sf: bool):
    ...
    payload = {
        ...
        "policy_loss": "complete official XR-1 flow/frequency/choice/score objective",
        ...
        "lr": LR, "weight_decay": WEIGHT_DECAY, "optimizer": "official DeepSpeed FusedAdam", "scheduler": "official cosine warmup",
...
def main():
    ...
    p.add_argument("--manifest", type=Path, default=Path("/data-cfs/data3/shurenbin/Xiaomi-Robotics-Spatial/train/manifests/robocasa365_sf10/train.jsonl"))
    p.add_argument("--dataset-root", type=Path, default=Path("/data-cfs/data3/datasets/robocasa365-datasets/pretrain"))
    p.add_argument("--model-ckpt", type=Path, default=Path("/data-cfs/data3/models/Xiaomi-Robotics-1-5B/model_states.pt"))
    p.add_argument("--processor", type=Path, default=Path(__file__).resolve().parent / "processor")
    p.add_argument("--esm-checkpoint", type=Path, default=Path("/data-cfs/data3/shurenbin/Xiaomi-Robotics-Spatial/train/esm_sink_token/runs/esm1b_robocasa365_geometry_fresh_400k_p4_retry2/checkpoint_014000.pt"))
    p.add_argument("--falcon-root", type=Path, default=Path("/data-cfs/data3/shurenbin/Xiaomi-Robotics-Spatial/train/esm_sink_token/vendor/FALCON"))
    p.add_argument("--output-dir", type=Path, default=Path(__file__).resolve().parent / "runs" / "spatial_forcing")
    p.add_argument("--checkpoint-dir", type=Path, default=Path(__file__).resolve().parent / "checkpoints" / "spatial_forcing")
    p.add_argument("--log-dir", type=Path, default=Path(__file__).resolve().parent / "logs" / "spatial_forcing")
    ...
    p.add_argument("--experiment", choices=["SF_qwen_L24", "SF_qwen_L27", "SF_qwen_L32"], default="SF_qwen_L27")
    p.add_argument("--alpha", type=float, default=0.5)
    p.add_argument("--alignment-layer", type=int, default=27)
    ...
    p.add_argument("--alignment-overfit-test", action="store_true")
    p.add_argument("--overfit-steps", type=int, default=500)
    p.add_argument("--negative-controls", action="store_true")
    p.add_argument("--use-vlm-norm", action="store_true")
    p.add_argument("--acceptance-only", action="store_true")
    args = p.parse_args()
    locked = {"SF_qwen_L24": (24, 0.5), "SF_qwen_L27": (27, 0.5), "SF_qwen_L32": (32, 0.5)}[args.experiment]
    if args.alignment_layer != locked[0] or abs(args.alpha - locked[1]) > 1e-12:
        raise ValueError(f"{args.experiment} locks alignment_layer={locked[0]} and alpha={locked[1]}")
    common.DATA_ROOT = args.dataset_root
    common.MANIFEST = args.manifest
    common.META = args.manifest.parent / "train.meta.json"
    ...
    write_contract(args.output_dir, args, rows, True)
    print(json.dumps({
        "mode": "spatial_forcing_posttrain", "base_checkpoint": str(args.model_ckpt),
        "robo_casa365_checkpoint_used": False, "esm_checkpoint": str(args.esm_checkpoint),
        ...
        "policy_views": ["LEFT", "RIGHT", "WRIST"], "spatial_views": ["LEFT", "RIGHT", "WRIST"],
        "spatial_current_only": True, "qwen_crop_ratio": 0.95, "esm_crop": False,
        "esm_coordinate_warp": "full 37x37 -> official 0.95 crop via bilinear sampling",
        "loss": "official policy loss + alpha * tokenwise per-view alignment loss",
        "alpha": args.alpha, "alignment_layer": args.alignment_layer,
    }), flush=True)
    ...
    if args.alignment_overfit_test:
        ...
        return
    data = SixTaskData(rows, processor, args.batch_size, args.workers, True, args.falcon_root, args.gpu_prefetch, stats_processor)
    runner = ForcingRunner(args, processor, sf=True)
    from lightning import Trainer
    from lightning.pytorch.callbacks import ModelCheckpoint
    from lightning.pytorch.strategies import DeepSpeedStrategy
    checkpoint = ModelCheckpoint(
        dirpath=str(args.checkpoint_dir), every_n_train_steps=1000,
        save_top_k=-1, save_last=True, enable_version_counter=False,
    )
    callbacks = [MemoryCallback(), checkpoint, common.SuccessEvaluationCallback(args.success_eval_json)]
    max_steps = 2 if args.acceptance_only else args.max_steps
    trainer = Trainer(
        accelerator="cuda", devices=args.devices, num_nodes=1, precision="bf16-mixed",
        strategy=DeepSpeedStrategy(), max_steps=max_steps, default_root_dir=str(args.log_dir),
        accumulate_grad_batches=args.gradient_accumulation, gradient_clip_val=1.0,
        log_every_n_steps=10, val_check_interval=1000, check_val_every_n_epoch=None,
        limit_val_batches=args.val_batches, num_sanity_val_steps=0, callbacks=callbacks,
